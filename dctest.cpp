...
* Based on FOX test of the same name, but modified to allow comparison          *
* between FXDCWindow and FXDCCairo.                                             *
*********************************************************************************
* $Id: $                          *
********************************************************************************/
...
public:
  ...
  void paintShapes(FXDCWindow & dc, FXCanvas * canvas);
...
public:
  ...
  long onCmdCairo(FXObject*,FXSelector,void*);
  long onUpdCairo(FXObject*,FXSelector,void*);
public:
  enum{
...
FXDEFMAP(DCTestWindow) DCTestWindowMap[]={
  ...
  FXMAPFUNC(SEL_COMMAND,  DCTestWindow::ID_CAIRO,                                           DCTestWindow::onCmdCairo),
  FXMAPFUNC(SEL_UPDATE,   DCTestWindow::ID_CAIRO,                                           DCTestWindow::onUpdCairo),
...
// Make some windows
DCTestWindow::DCTestWindow(FXApp *app):FXMainWindow(app,"Cairo Device Context Test",NULL,NULL,DECOR_ALL,0,0,900,900){
  ...
  new FXCheckButton(controls,"Use FXDCCairo",this,ID_CAIRO);
  ...
  birdImage=new FXPNGImage(getApp(),car500w);
...
long DCTestWindow::onCmdCairo(FXObject*sender,FXSelector sel,void*){
  FXival x;
  sender->handle(this,FXSEL(SEL_COMMAND,ID_GETINTVALUE),&x);
  use_cairo = (FXbool)x;
  linesCanvas->update();
  shapesCanvas->update();
  imagesCanvas->update();
...
long DCTestWindow::onUpdCairo(FXObject*sender,FXSelector sel,void*){
  sender->handle(this,FXSEL(SEL_COMMAND,ID_UNCHECK+use_cairo),NULL);
  return 1;
  }
...
void DCTestWindow::paintShapes(FXDCWindow & dc, FXCanvas * canvas) {
...
void DCTestWindow::drawPage(FXDC& dc,FXint w,FXint h){
  ...
  dc.fillRectangle(30,h-70,4,4);
  dc.fillRectangle(30,h-30,4,4);
  dc.fillRectangle(30,h-90,4,4);
  dc.fillRectangle(70,h-90,4,4);
